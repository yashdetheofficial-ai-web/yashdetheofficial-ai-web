...
const oldCond = 'if (q.includes("overall") || q.includes("sagle") || q.includes("sagli") || q.includes("total") || q.includes("all medicine") || q.includes("price list") || q.includes("सगळे दर") || q.includes("सर्व औषधे") || q.includes("सगळी औषधे") || q.includes("कोंबडी औषध yadi")) {';
const newCond = 'if (q.includes("overall") || q.includes("sagle") || q.includes("sagli") || q.includes("total") || q.includes("all medicine") || q.includes("price list") || q.includes("सगळे दर") || q.includes("सर्व औषधे") || q.includes("सगळी औषधे") || q.includes("कोंबडी औषध yadi") || q.includes("live rate") || q.includes("yadi") || q.includes("यादी")) {';
...
const endIndex = content.indexOf(oldOverallEnd) + oldOverallEnd.length;

if (startIndex !== -1 && endIndex !== -1) {
  content = content.slice(0, startIndex) + newOverall + content.slice(endIndex);
  console.log("Replaced overall display logic successfully.");
} else {
  console.log("Failed to replace overall display logic.");
}
...
if (content.includes('  }\\n];')) {
    ...
    console.log("Added new medicines successfully.");
    ...
    // try exact regex replacement
    content = content.replace(/  \}\r?\n\];/g, '  },\n' + newMeds);
    console.log("Attempted regex replacement for medicines.");
}
