...
file_path = r'c:\Users\YASHUUU\Desktop\Dethe patil\src\components\MedicineChatbot.jsx'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# 1. Update overall search condition
old_cond = 'if (q.includes("overall") || q.includes("sagle") || q.includes("sagli") || q.includes("total") || q.includes("all medicine") || q.includes("price list") || q.includes("सगळे दर") || q.includes("सर्व औषधे") || q.includes("सगळी औषधे") || q.includes("कोंबडी औषध yadi")) {'
new_cond = 'if (q.includes("overall") || q.includes("sagle") || q.includes("sagli") || q.includes("total") || q.includes("all medicine") || q.includes("price list") || q.includes("सगळे दर") || q.includes("सर्व औषधे") || q.includes("सगळी औषधे") || q.includes("कोंबडी औषध yadi") || q.includes("live rate") || q.includes("yadi") || q.includes("यादी")) {'
content = content.replace(old_cond, new_cond)

# 2. Update overall search display
old_overall_start = 'let overallResponse = `<div class="p-4 bg-emerald-500/10 border-l-4 border-emerald-500 rounded-lg mb-4">'
old_overall_end = 'setIsTyping(false);\n        return;\n      }'

new_overall = """let overallResponse = `
                  ...
                  <div class="group bg-slate-900/60 backdrop-blur-xl p-5 rounded-[2rem] border border-slate-700/50 hover:border-emerald-500/40 hover:bg-slate-800/80 transition-all duration-500 shadow-xl hover:shadow-emerald-500/10">
                    <div class="flex justify-between items-start gap-4">
                      ...
                      <div class="bg-emerald-950/40 px-4 py-3 rounded-2xl border border-emerald-500/30 text-center min-w-[95px] flex flex-col justify-center group-hover:bg-gradient-to-br group-hover:from-emerald-500 group-hover:to-teal-600 group-hover:scale-105 transition-all duration-300 shadow-lg group-hover:shadow-emerald-500/30">
                        <p class="text-[8px] text-emerald-400 font-bold uppercase mb-0.5 group-hover:text-emerald-50 tracking-widest">Live Rate</p>
...
match = re.search(re.escape(old_overall_start) + '.*?' + re.escape(old_overall_end), content, re.DOTALL)
if match:
    content = content[:match.start()] + new_overall + content[match.end():]
    print("Replaced overall display logic successfully.")
else:
    print('Failed to replace overall display logic.')
...
if '  }\n];' in content:
    ...
    print("Added new medicines successfully.")
else:
    print("Failed to add new medicines.")
